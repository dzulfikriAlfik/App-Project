...
function buildTemplate(data, index, page, perPage) {
   let rows = `
      <tr class='template-data'>
         ...
         <td>
            Lebar : ${data[index].ads_type_width},
            Tinggi : ${data[index].ads_type_height}
         </td>
...
function search(page) {
   ...
   let params = `&ads_type_name=${filterAdsType}&ads_type_status=${filterAdsTypeStatus}`;
...
function save() {
   ...
   if ($("#ads_type_id").val() == "") {
      ...
      commonAPI.putAPI(
         "/api/ads_type/put/" + $("#ads_type_id").val(),
         data,
         function (response) {
            commonJS.loading(false);
