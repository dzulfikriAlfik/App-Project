...
class AdminController extends Controller
{
  ...
  public function chatUser(User $user)
  {
    $data = [
      "user"       => auth()->user(),
      "mitra"      => $user,
      "chat_mitra" => Chat::where("id_mitra", $user->id)->orderBy("id_chat", "ASC")->get()
    ];

    return view("chat.mitra", $data);
  }

  public function sendChat(User $user)
  {
    $data = [
      "user"  => auth()->user(),
      "mitra" => $user,
  ...
  public function storeChat(Request $request, User $user)
  {
    ...
    $validatedData["id_mitra"] = $user->id;

    Chat::create($validatedData);

    return redirect("/mitra/chat/{$user->id}")->with('success', 'Chat berhasil ditambahkan!');
  }
  ...
  public function deleteChat(Chat $chat, User $user)
  {
    ...
    return redirect("/mitra/chat/{$user->id}")->with('success', 'Chat berhasil dihapus!');
  }
}
