...
function getUsersStatistic() {
  var url = `${baseUrl}/api/user/get/statistic`;
  ...
  commonAPI.getAPI(url, (response) => {
    ...
    if (response.status == 200) {
      ...
      $("#total_users").text(commonJS.numberFormat(data.all_time) + " (Total)");
...
function getUserGrowthStatistic(days) {
  var url = `${baseUrl}/api/stat/chart?days=` + days;
  commonJS.loading(true);
  commonAPI.getAPI(url, (response) => {
    // console.log(response)
    if (response.status == 200) {
