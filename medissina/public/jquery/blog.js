...
function buildTemplate(data, index, page, perPage) {
  ...
  if (data[index].is_draft == 1) {
    rows +=
      "<button type='button' class='btn btn-light btn-xs dropdown-item' onClick='publish(\"" +
      data[index].id +
      "\")' style='margin-right: 5px;'><i class='fa fa-eye feather-16'></i> Publish</button>";
  } else if (data[index].is_draft == 0) {
    rows +=
      "<button type='button' class='btn btn-light btn-xs dropdown-item' onClick='draft(\"" +
      data[index].id +
      "\")' style='margin-right: 5px;'><i class='fa fa-eye feather-16'></i> Unpublish</button>";
  }
  rows +=
...
function save(draft) {
  ...
  if ($("#blog_title").val() == "") {
    ...
    commonJS.showErrorMessage(
      "#msgBox",
      commonMsg.getMessage(["Blog Title"], commonMsg.MSG_REQUIRED)
    );
    return;
  }
  ...
  if (CKEDITOR.instances["article-ckeditor"].getData() == "") {
    ...
    commonJS.showErrorMessage(
      "#msgBox",
      commonMsg.getMessage(["Blog Content"], commonMsg.MSG_REQUIRED)
    );
    return;
  }
  ...
  if ($("#id").val() == "") {
    ...
    if ($("#blog_img").val() == "") {
      ...
      commonJS.showErrorMessage(
        "#msgBox",
        commonMsg.getMessage(["Blog Image Cover"], commonMsg.MSG_REQUIRED)
      );
      return;
    }
    // Validate type file
    ...
    if ($("#blog_img")[0].files[0].type.split("/")[0] != "image") {
      ...
      commonJS.showErrorMessage(
        "#msgBox",
        commonMsg.getMessage(["File"], commonMsg.MSG_ONLY_IMG)
      );
      return;
    }
    // validate file size
    if ($("#blog_img")[0].files[0].size > 10000000) {
      ...
      commonJS.showErrorMessage(
        "#msgBox",
        commonMsg.getMessage(["Blog Image Cover"], commonMsg.MSG_MORE_THAN_1MB)
      );
      return;
    }
  }
